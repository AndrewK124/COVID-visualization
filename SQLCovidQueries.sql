...
post_vax_deaths AS (
	...
	SELECT
		d.country, 
		SUM(d.new_deaths) AS total_post_vax_deaths, -- Total deaths after reaching 50% vaccination coverage 
		COUNT(DISTINCT d.date) AS num_days_post_vax, -- Number of days post 50% vaccination coverage 
		MAX(d.population) AS population -- Maximum recorded popluation for scaling per million
	FROM 
		CovidDeaths d 
	JOIN 
		vax_threshold_date v ON d.country = v.country 
	WHERE 
...
SELECT 
	...
	p.total_post_vax_deaths, -- Total deaths after 50% vaccination threshold
	p.num_days_post_vax, -- Number of days after 50% vaccination threshold
	(p.total_post_vax_deaths * 1.0 / p.num_days_post_vax) AS avg_daily_deaths_post_vax, -- Average daily deaths post 50% vaccination coverage
	CASE 
		WHEN p.population IS NOT NULL AND p.population > 0 
			THEN (p.total_post_vax_deaths * 1.0 / p.population) * 1000000 -- Deaths per million post 50% vaccination coverage
		ELSE NULL
	END AS deaths_per_million_post_vax
FROM 
	latest_vax lv
JOIN 
...
pre_post_reproduction AS ( 
	SELECT 
		...
		AVG(CASE 
			...
			THEN TRY_CAST(d.reproduction_rate AS float) END) AS avg_COVID_reproduction_pre,  
		AVG(CASE 
			WHEN d.date >= r.rollout_date 
			AND d.date < DATEADD(day, 30, r.rollout_date) 
			THEN TRY_CAST(d.reproduction_rate AS float) END) AS avg_COVID_reproduction_post
	FROM 
		CovidDeaths d 
	JOIN 
...
SELECT *, (avg_COVID_reproduction_post - avg_COVID_reproduction_pre) / avg_COVID_reproduction_pre * 100 AS reproduction_percent_change 
...
continent_peaks AS ( 
	SELECT d.continent, d.date AS peak_date, p.peak_cases 
	FROM CovidDeaths d 
	JOIN continent_peak_cases p ON d.continent = p.continent AND d.new_cases = p.peak_cases 
	WHERE ISDATE(d.date) = 1
), 
continent_vax_threshold AS ( 
	SELECT 
		v.continent, MIN(CAST(v.date AS date)) AS vax_threshold_date
	FROM 
		CovidVaccinations v 
	WHERE 
...
SELECT  
	p.continent, 
	p.peak_date, 
	v.vax_threshold_date, 
	DATEDIFF(day, v.vax_threshold_date, p.peak_date) AS days_difference, 
	CASE
		WHEN DATEDIFF(day, v.vax_threshold_date, p.peak_date) > 0 THEN '50% of population vaxxed before peak COVID cases'
		WHEN DATEDIFF(day, v.vax_threshold_date, p.peak_date) < 0 THEN '50% of population vaxxed after peak COVID cases'
		ELSE '50% of population vaxxed same day as peak COVID cases' 
	END AS peak_timing
FROM 
	continent_peaks p 
JOIN 
	continent_vax_threshold v ON p.continent = v.continent 
ORDER BY 
	days_difference ASC;
...
before_after_cases AS ( 
	SELECT 
		c.country, 
		p.peak_cases_date, 
		AVG(CASE WHEN c.date BETWEEN DATEADD(day, -14, p.peak_cases_date) AND DATEADD(day, -1, p.peak_cases_date) THEN c.new_cases_per_million END) as avg_cases_before, 
		AVG(CASE WHEN c.date BETWEEN p.peak_cases_date AND DATEADD(day, 14, p.peak_cases_date) THEN c.new_cases_per_million END) AS avg_cases_after 
	FROM 
		CovidDeaths c 
	JOIN 
		selected_peaks p ON c.country = p.country 
	WHERE 
...
SELECT 
	...
	b.avg_cases_before, 
	b.avg_cases_after, 
	CASE WHEN b.avg_cases_before = 0 THEN NULL ELSE ((b.avg_cases_after - b.avg_cases_before) / b.avg_cases_before) * 100 END AS cases_percent_change 
FROM
	before_after_cases b
JOIN 
	selected_peaks s ON b.country = s.country 
ORDER BY 
